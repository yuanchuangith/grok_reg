...
项目内置本地 Web 管理台，兼容现有 `config.json`、邮箱池、账号账本和
`cpa_auths` 文件，不迁移已有数据。

```powershell
uv run python -u web_dashboard.py --open
```

Windows 也可以直接双击 `Start Web Dashboard.cmd`。默认只监听
`http://127.0.0.1:8765`。
...
- macOS 风格运行总览与实时日志
- 四段格式邮箱凭证导入、更新和删除
- 成功账户、SSO 与 CPA 凭证管理和导出
- 注册失败、CPA 失败记录筛选与重试标记清理
- 新账户注册、存量账户补 CPA 任务
- `config.json` 全配置项分组编辑与自动备份

### 代理与 Clash 订阅池

配置中心支持三种出口模式：

- `direct`：关闭代理，浏览器、邮箱请求和 CPA 全部直连
- `static`：使用 `proxy` / `cpa_proxy` 固定代理
- `clash_subscription`：由 Mihomo 加载 Clash 订阅，通过本地 mixed-port 提供统一代理

订阅池模式会在每个任务开始前随机选择节点，并让该节点实际访问
`proxy_pool_health_url`。节点无法访问或超时时会自动尝试下一个，直到找到
可用节点或达到 `proxy_pool_max_test_nodes` 上限。由于 Mihomo 的选择组是共享
出口，订阅池随机节点模式会自动将注册并发限制为 1，避免多个账户运行时互相切换出口。
