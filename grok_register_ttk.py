...
DEFAULT_CONFIG = {
    ...
    "proxy_pool_host": "127.0.0.1",
    "proxy_pool_mixed_port": 17890,
    "proxy_pool_controller_host": "127.0.0.1",
    "proxy_pool_controller_port": 19090,
    "proxy_pool_health_url": "https://accounts.x.ai/",
    "proxy_pool_test_timeout_sec": 8,
    "proxy_pool_max_test_nodes": 12,
    "proxy_pool_refresh_seconds": 3600,
    "proxy_pool_health_interval_seconds": 120,
    "proxy_pool_provider_name": "subscription",
    "proxy_pool_group_name": "PROXY_POOL",
    "proxy_pool_mihomo_path": "",
    "proxy_pool_runtime_dir": "./proxy_pool_runtime",
    "enable_nsfw": True,
    "register_count": 1,
...
def get_proxies():
    proxy = proxy_pool.effective_proxy(config, purpose="register")
    if proxy:
        return {"http": proxy, "https": proxy}
    return {}


def prepare_outbound_proxy(purpose="register", target=None, log_callback=None):
    """Validate/select a live proxy before an account workflow starts."""
    return proxy_pool.prepare_proxy(
        config,
        purpose=purpose,
        target=target,
        log=log_callback,
...
def http_get(url, **kwargs):
    try:
        ...
        err = str(exc)
        # Only fall back to direct when explicitly enabled. Pool mode rotates
        # dead nodes before the workflow starts instead of silently leaking to direct.
        if config.get("proxy_fallback_direct", False) and ("Could not connect to server" in err or "Failed to connect" in err):
...
def click_email_signup_button(timeout=10, log_callback=None, cancel_callback=None):
    ...
    def email_form_ready():
        try:
            return bool(
                page.run_js(
                    """
const nodes = Array.from(document.querySelectorAll('input[data-testid="email"], input[name="email"], input[type="email"], input[autocomplete="email"]'));
return nodes.some((node) => {
  ...
  return style.display !== 'none' && style.visibility !== 'hidden' && rect.width > 0 && rect.height > 0 && !node.disabled;
    ...
    while time.time() < deadline:
        ...
        if email_form_ready():
            if log_callback:
                log_callback("[REG-EMAIL-103] 邮箱注册表单已经可见")
            return True
        if log_callback:
            log_callback("[REG-EMAIL-100] 查找“使用邮箱注册”按钮")

        native_target = None
        try:
            for element in page.eles("tag:button"):
                text = "".join(str(element.text or "").lower().split())
                if text in {"signupwithemail", "continuewithemail", "使用邮箱注册", "使用邮箱继续"}:
                    if bool(element.states.is_displayed):
                        native_target = element
                        break
        except Exception as exc:
            if log_callback:
                log_callback(f"[REG-EMAIL-102] 原生按钮查找失败 reason={exc}")

        if native_target is not None:
            try:
                if log_callback:
                    log_callback(f"[REG-EMAIL-101] 使用浏览器原生事件点击按钮 text={native_target.text}")
                native_target.click(by_js=False, timeout=4)
                verify_deadline = min(deadline, time.time() + 5)
                while time.time() < verify_deadline:
                    raise_if_cancelled(cancel_callback)
                    if email_form_ready():
                        if log_callback:
                            log_callback("[REG-EMAIL-103] 原生点击成功，邮箱输入框已经出现")
                        return True
                    human_sleep(0.25, cancel_callback)
                if log_callback:
                    log_callback("[REG-EMAIL-104] 原生点击后表单未出现，尝试脚本点击兜底")
            except Exception as exc:
                if log_callback:
                    log_callback(f"[REG-EMAIL-102] 原生点击失败 reason={exc}")
...
const target = candidates.find((node) => {
    ...
    return (
        ...
        lower === 'signupwithemail' ||
        lower === 'continuewithemail' ||
        lower === '使用邮箱继续'
    );
});
if (!target) {
...
return true;
        ...
        if clicked:
            if log_callback:
                log_callback("[REG-EMAIL-105] 已执行脚本点击，等待邮箱表单确认")
            verify_deadline = min(deadline, time.time() + 3)
            while time.time() < verify_deadline:
                raise_if_cancelled(cancel_callback)
                if email_form_ready():
                    if log_callback:
                        log_callback("[REG-EMAIL-106] 脚本点击成功，邮箱输入框已经出现")
                    return True
                human_sleep(0.25, cancel_callback)
            if log_callback:
                log_callback("[REG-EMAIL-107] 脚本点击未触发页面切换")
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            for element in page.eles("tag:input"):
                input_type = str(element.attr("type") or "").lower()
                input_name = str(element.attr("name") or "").lower()
                test_id = str(element.attr("data-testid") or "").lower()
                autocomplete = str(element.attr("autocomplete") or "").lower()
                if (
                    (input_type == "email" or input_name == "email" or test_id == "email" or autocomplete == "email")
            ...
            if native_input is not None:
                if log_callback:
                    log_callback("[REG-EMAIL-201] 找到邮箱输入框，使用浏览器原生键盘事件输入")
                native_input.input(email, clear=True, by_js=False)
                current_value = str(native_input.run_js("return this.value") or "").strip()
                if current_value == email:
                    if log_callback:
                        log_callback("[REG-EMAIL-202] 邮箱输入完成，查找提交按钮")
                    submit_target = None
                    for element in page.eles("tag:button"):
                        text = "".join(str(element.text or "").lower().split())
                        if (
                            (text in {"注册", "signup", "continue", "next", "下一步", "继续"} or "signup" in text)
                            and bool(element.states.is_displayed)
                        ):
                            submit_target = element
                            break
                    if submit_target is not None:
                        submit_target.click(by_js=False, timeout=4)
                        if log_callback:
                            log_callback(f"[REG-EMAIL-203] 已使用原生事件提交邮箱 {email}")
                        dump_state(page, "email-submitted")
                        take_screenshot(page, "email-submitted")
                        return email, dev_token
        except Exception as exc:
            if log_callback:
                log_callback(f"[REG-EMAIL-204] 原生输入或提交失败，转入脚本兜底 reason={exc}")
...
return input.value;
        ...
        if filled != "filled":
            if log_callback:
                log_callback(f"[REG-EMAIL-205] 脚本写入邮箱失败 result={filled}")
            human_sleep(0.5, cancel_callback)
            continue
...
return true;
        ...
        if clicked:
            if log_callback:
                log_callback(f"[REG-EMAIL-206] 已使用脚本兜底提交邮箱 {email}")
            dump_state(page, "email-submitted")
            take_screenshot(page, "email-submitted")
            return email, dev_token
        human_sleep(0.5, cancel_callback)
