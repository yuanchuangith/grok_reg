...
def export_cpa_xai_for_account(
    ...
    *,
    ...
    # Explicit proxy mode: disabled/direct, static, or local Clash pool.
    proxy = proxy_pool.effective_proxy(cfg, purpose="cpa")
    if not proxy and "proxy_enabled" not in cfg:
        proxy = (
    ...
    if proxy_pool.proxy_is_enabled(cfg, purpose="cpa"):
        selected = proxy_pool.prepare_proxy(
            cfg,
            purpose="cpa",
            target=proxy_pool.health_target(cfg, purpose="cpa"),
            log=lambda message: log(f"[cpa] [proxy] {message}"),
        )
        proxy = str(selected.get("proxy") or proxy_pool.effective_proxy(cfg, purpose="cpa"))
