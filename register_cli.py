...
def register_one(
    ...
    *,
    ...
    try:
        selected = reg.prepare_outbound_proxy(
            purpose="register",
            log_callback=lambda m: log(worker_id, f"[proxy] {m}"),
        )
        label = selected.get("node") or selected.get("mode") or "direct"
        log(worker_id, f"[proxy] ready: {label}")
    except Exception as exc:
        log(worker_id, f"! 代理验活失败: {exc}")
        _inc("reg_fail")
        return None
...
def main() -> int:
    ...
    if (
        bool(cfg0.get("proxy_enabled", bool(cfg0.get("proxy"))))
        and reg.proxy_pool.selection_mode(cfg0) == "random"
        and threads > 1
    ):
        print("[proxy] 随机代理模式使用全局出口，注册并发已自动限制为 1", flush=True)
        threads = 1
